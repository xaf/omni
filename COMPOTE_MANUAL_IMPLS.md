...
This inventory is derived from the uncommented `compote::FromContextValue`
...
| 7 | `UpConfig` | `src/internal/config/up/base.rs` | Parses a heterogeneous step array, normalizes bare tool names, treats numeric YAML values as mise tool names, detects empty operations, and collects both Compote diagnostics and omni `UpError` values while continuing. |
...
| 10 | `StoredConfig` | `src/internal/config/parser/suggest_config.rs` | Intentionally accepts any value and strips its context into a context-free `compote::Value`. |
...
```text
^\s*impl<.*compote::FromContextValue
```
...
`#[derive(compote::Config)]` and are not part of the count:
