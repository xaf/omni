...
impl SandboxCommand {
    ...
    fn write_config(&self, target: &Path, dependencies: &[String]) -> Result<(), String> {
        ...
        if config_path.exists() {
            if dependencies.is_empty() {
                // Nothing to do
                return Ok(());
            } else if self.confirm_add_dependencies(&config_path) {
                return self.add_dependencies_to_config(&config_path, dependencies);
            }
            return Ok(());
        } else {
            if dependencies.is_empty() {
    ...
    fn confirm_add_dependencies(&self, _config_path: &Path) -> bool {
        ...
        let question = requestty::Question::confirm("add_dependencies")
            ...
            .message("An .omni.yaml file already exists, add dependencies to it?")
            .default(true)
            .build();
    ...
    fn confirm_continue_with_existing(&self, reason: &str) -> Result<(), String> {
        if !shell_is_interactive() {
            return Err(format!("directory {}", reason));
        }

        let question = requestty::Question::confirm("continue_existing")
            .ask_if_answered(true)
            .on_esc(requestty::OnEsc::Terminate)
            .message(format!("The directory {}. Continue anyway?", reason))
            .default(false)
            .build();
    ...
    fn initialize_at(
        ...
        if target.exists() {
            ...
            // Check if directory already contains a work directory
            if target.join(".omni").join("id").exists() {
                self.confirm_continue_with_existing("already contains a work directory")?;
            }

            // Check if directory already contains a git repository
            if target.join(".git").is_dir() {
                self.confirm_continue_with_existing("already contains a git repository")?;
            }
